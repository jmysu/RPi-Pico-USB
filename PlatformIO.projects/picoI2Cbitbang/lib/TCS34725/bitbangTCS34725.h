...
public:
    ...
    //bool attach(i2cType& w = i2c)
    bool attach(BBI2C *bbi2c)
    {
        //i2c = &w;
        i2c = bbi2c;
    ...
    {
        //i2c->beginTransmission(I2C_ADDR);
        //i2c->write(COMMAND_BIT | (uint8_t)reg);
        //i2c->write(value);
        //i2c->endTransmission();
...
private:
    ...
    {   /*
        i2c->beginTransmission(I2C_ADDR);
        i2c->write(COMMAND_BIT | (uint8_t) Reg::CDATAL);
        i2c->endTransmission();
        i2c->requestFrom(I2C_ADDR, sizeof(RawData));
        for (uint8_t i = 0; i < sizeof(RawData); i++)
            raw_data.raw[i] = i2c->read();
        */
